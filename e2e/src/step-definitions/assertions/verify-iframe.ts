...
Then(
    ...
    async function(this: ScenarioWorld, elementKey: string, iframeName: string, negate: boolean) {
        const {
            screen: { page },
            globalConfig,
        } = this;
...
Then(
    ...
    async function(this: ScenarioWorld, elementKey: ElementKey, iframeName: string, negate: boolean, expectedElementText: string) {
        ...
        console.log(`the ${elementKey} on the ${iframeName} should ${negate?'not':''}equal the text ${expectedElementText}`)
